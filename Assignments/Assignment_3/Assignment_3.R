...
Sepal.Area>20
class(Sepal.Area)
as.character(Sepal.Area)
class(Sepal.Area)
Sepal.Area%in% 20 #why cant I do this and find exact values of 20?
C(20)%in% Sepal.Area #I need to change it to a factor?
as.factor(Sepal.Area)
class(Sepal.Area)#I cannot change it from Numeric!
class(iris$Sepal.Area)
as.factor(iris$Sepal.Area)
class(iris$Sepal.Area)#Still numeric!
factor(c(Sepal.Area))
class(Sepal.Area)

factor(c(iris$Sepal.Area))
class(iris$Sepal.Area)
V3<- as.factor(Sepal.Area)
class(V3)
V3 >= 20 #needs to be a numeric class
V3%in% 20:100
class(Sepal.Area)
big_area_iris <- Sepal.Area >=20
data.frame(big_area_iris)

sub1<- Sepal.Area[Sepal.Area>=20]  #Brackets got me the specific values I wanted
big_area_iris <- data.frame(sub1)
